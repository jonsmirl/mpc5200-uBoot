...
int do_mem_test ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
  ...
  usage:
	...
	printf(" Memory Test(0x%x - 0x%x): WR Start\n", start, end);
	...
	while(addr <= end) {
		...
		*addr = 0x00000000; data = *addr;
		if(data != 0x00000000) printf("FAIL WR: 0x%x= 0x%x (0x00000000 excepted).\n", addr, data);

		*addr = 0xffffffff; data = *addr;
		if(data != 0xffffffff) printf("FAIL WR: 0x%x= 0x%x (0xffffffff excepted).\n", addr, data);

		*addr = 0x55555555; data = *addr;
		if(data != 0x55555555) printf("FAIL WR: 0x%x= 0x%x (0x55555555 excepted).\n", addr, data);

		*addr = 0xaaaaaaaa; data = *addr;
		if(data != 0xaaaaaaaa) printf("FAIL WR: 0x%x= 0x%x (0xaaaaaaaa excepted).\n", addr, data);

		*addr = 0xaa55aa55; data = *addr;
		if(data != 0xaa55aa55) printf("FAIL WR: 0x%x= 0x%x (0xaa55aa55 excepted).\n", addr, data);

		*addr = 0x12345678; data = *addr;
		if(data != 0x12345678) printf("FAIL WR: 0x%x= 0x%x (0x12345678 excepted).\n", addr, data);
	...
	printf(" Memory Test(0x%x - 0x%x): RD Start\n", start, end);
	...
	if(memError == 0) printf("Memory Test Success\n");
